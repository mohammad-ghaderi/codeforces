...
void solve() {
    ...
    int both_pos = 0, both_neg = 0, first = 0, second = 0;

    for (int i = 0; i < n; i++) {
        if (p[0][i] == 1 and p[1][i] == 1) both_pos++;
        else if (p[0][i] == -1 and p[1][i] == -1) both_neg++;
        else if (p[0][i] == 1) first++;
        else if (p[1][i] == 1) second++;         
    }
